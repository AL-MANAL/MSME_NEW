	...
	var _componentWizard = function () {
		...
		$('.steps-basic').steps({
			...
			labels: {
				...
				finish: 'Submit RCA details <i class="icon-arrow-right14 ml-2" />'
			},
		...
		$('.steps-validation').steps({
			...
			labels: {
				previous: '<i class="icon-arrow-left13 mr-2" /> Previous',
				next: 'Next <i class="icon-arrow-right14 ml-2" />',
                finish: '<text title="Submit Root Cause Analysis Details"> Submit RCA details <i class="icon-arrow-right14 ml-2" /></text>'
				...
				jQuery.ajax({
					...
					success: function (result) {
						...
						window.location = window.location.protocol + "//" +
                            ...
                            window.location.pathname.replace("AddRCA", "CustomerComplaintList");
		...
		// Show form
		var forms = $('.steps-validation-edit').show();


		// Initialize wizard
		$('.steps-validation-edit').steps({
			headerTag: 'h6',
			bodyTag: 'fieldset',
			titleTemplate: '<span class="number">#index#</span> #title#',
			labels: {
			...
			onStepChanging: function (event, currentIndex, newIndex) {
				...
				if (currentIndex < newIndex) {
					...
					// To remove error styles
					forms.find('.body:eq(' + newIndex + ') label.error').remove();
					forms.find('.body:eq(' + newIndex + ') .error').removeClass('error');
				}
		...
		//// Initialize validation
		$('.steps-validation-edit').validate({
			ignore: 'input[type=hidden], .select2-search__field', // ignore hidden fields
			errorClass: 'validation-invalid-label',
			highlight: function (element, errorClass) {
				$(element).removeClass(errorClass);
